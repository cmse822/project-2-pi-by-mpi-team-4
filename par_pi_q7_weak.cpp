...
int main(int argc, char *argv[])
{
   ...
   double avepi,localavepi;       	/* average pi value for all iterations */
   int i, n;
   double start_time, end_time, elapsed_time;
   int numtasks, rank, numnodes;
   ...
   numnodes = stod(argv[3]);  // Convert command-line argument to an integer
   ...
   if (rank == dest) {
      ...
      std::ifstream infile(csvfile);
         ...
         if (is_empty) {
               file << "Darts, Rounds, # nodes, # proc, pi, time, error" << std::endl;
         }

         // Write results to the CSV file
         if (file.is_open()) {
               file << DARTS << ',' << ROUNDS << ',' << numnodes << ',' << numtasks << ',' << globalmean << ',' << elapsed_time << ',' << pierror << std::endl;
               file.close();
         } else {
